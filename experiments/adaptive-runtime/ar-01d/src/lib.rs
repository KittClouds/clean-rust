...
pub enum Arm {
    ...
    D2SameFourCommits,
    D3SameEightCommits,
    D4IndependentOneCommit,
    D5IndependentTwoCommits,
    D6IndependentFourCommits,
    D7IndependentEightCommits,
    D8FullOneCommit,
    D9FullTwoCommits,
    D10FullFourCommits,
    D11FullEightCommits,
...
impl Arm {
    pub const ALL: [Self; 14] = [
        Self::D0SameOneCommit,
        Self::D1SameTwoCommits,
        Self::D2SameFourCommits,
        Self::D3SameEightCommits,
        Self::D4IndependentOneCommit,
        Self::D5IndependentTwoCommits,
        Self::D6IndependentFourCommits,
        Self::D7IndependentEightCommits,
        Self::D8FullOneCommit,
        Self::D9FullTwoCommits,
        Self::D10FullFourCommits,
        Self::D11FullEightCommits,
        Self::D12AdamW,
        Self::D13Sign,
    ];
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::D0SameOneCommit => "D0-same-m1",
            Self::D1SameTwoCommits => "D1-same-m2",
            Self::D2SameFourCommits => "D2-same-m4",
            Self::D3SameEightCommits => "D3-same-m8",
            Self::D4IndependentOneCommit => "D4-independent-m1",
            Self::D5IndependentTwoCommits => "D5-independent-m2",
            Self::D6IndependentFourCommits => "D6-independent-m4",
            Self::D7IndependentEightCommits => "D7-independent-m8",
            Self::D8FullOneCommit => "D8-full-m1",
            Self::D9FullTwoCommits => "D9-full-m2",
            Self::D10FullFourCommits => "D10-full-m4",
            Self::D11FullEightCommits => "D11-full-m8",
            Self::D12AdamW => "D12-adamw",
            Self::D13Sign => "D13-sign",
        }
    }
    ...
    const fn beam(self) -> Option<usize> {
        match self {
            ...
            | Self::D9FullTwoCommits
            | Self::D10FullFourCommits
            | Self::D11FullEightCommits => Some(2),
            Self::D12AdamW | Self::D13Sign => None,
        }
    }
    ...
    const fn commits_per_evidence(self) -> usize {
        match self {
            Self::D0SameOneCommit | Self::D4IndependentOneCommit | Self::D8FullOneCommit => 1,
            Self::D1SameTwoCommits | Self::D5IndependentTwoCommits | Self::D9FullTwoCommits => 2,
            Self::D2SameFourCommits | Self::D6IndependentFourCommits | Self::D10FullFourCommits => {
                4
            }
            Self::D3SameEightCommits
            | Self::D7IndependentEightCommits
            | Self::D11FullEightCommits => 8,
            Self::D12AdamW | Self::D13Sign => 1,
        }
    }
    ...
    const fn verification(self) -> VerificationEvidence {
        match self {
            ...
            | Self::D1SameTwoCommits
            | Self::D2SameFourCommits
            | Self::D3SameEightCommits => VerificationEvidence::SameBatch,
            Self::D4IndependentOneCommit
            | Self::D5IndependentTwoCommits
            | Self::D6IndependentFourCommits
            | Self::D7IndependentEightCommits => VerificationEvidence::IndependentBatch,
            Self::D8FullOneCommit
            | Self::D9FullTwoCommits
            | Self::D10FullFourCommits
            | Self::D11FullEightCommits => VerificationEvidence::FullTrain,
            Self::D12AdamW | Self::D13Sign => VerificationEvidence::SameBatch,
        }
    }
}
...
pub fn run(samples: &[Sample], arm: Arm, seed: u64) -> RunSummary {
    ...
    let mut curve = Vec::with_capacity(STEPS / 10 + 1);

    for step in 0..STEPS {
        let proposal_indices = batch_indices(seed, step, 0x5052_4f50_4f53_414c);
        ...
        } else if arm == Arm::D13Sign {
            let (_, gradient) = model.gradient(&proposal_storage);
            update_sign(&mut model, &gradient);
        } else {
            let independent_indices = batch_indices(seed, step, 0x5645_5249_4659_5354);
            let independent_storage = indexed_samples(train, &independent_indices);
            let verify_storage: &[Sample] = match arm.verification() {
                VerificationEvidence::SameBatch => &proposal_storage,
                VerificationEvidence::IndependentBatch => &independent_storage,
                VerificationEvidence::FullTrain => train,
            };
            for commit_index in 0..arm.commits_per_evidence() {
                let (groups, group_count) = partition(step % PARAMS);
                ...
                if step % AUDIT_EVERY == 0 && commit_index == 0 {
    ...
    RunSummary {
        ...
        telemetry,
        coverage: coverage_summary(&coverage, if arm.runtime() { STEPS } else { 0 }),
...
mod tests {
    ...
    fn short_runtime_run_commits_a_program() {
        let dataset = spiral_dataset();
        let result = run(&dataset.samples, Arm::D2SameFourCommits, SEEDS[0]);
