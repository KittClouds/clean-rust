...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        artifact_dir.join("ar-01a-report.json"),
        render_report(&results),
    )?;
    fs::write(artifact_dir.join("ar-01a-runs.csv"), render_runs(&results))?;
    fs::write(
        artifact_dir.join("ar-01a-curves.csv"),
        render_curves(&results),
    )?;
    println!("AR-01A — stochastic 3-class spiral verification");
    println!(
...
fn render_report(results: &[RunSummary]) -> String {
    ...
    out.push_str("{\n  \"schema\": \"adaptive-runtime-ar-01a/v1\",\n  \"scope\": \"engineering-only\",\n  \"task\": \"three-class-spiral\",\n  \"protocol\": \"A0 AdamW; A1 sign; A2 K2 same-batch; A3 K5 same-batch; A4 K2 independent verification; 3000 minibatch steps; 3 seeds\",\n  \"results\": [\n");
