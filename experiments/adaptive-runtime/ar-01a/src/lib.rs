...
impl Arm {
    pub const ALL: [Self; 5] = [
        ...
        Self::A2K2SameBatch,
        Self::A3K5SameBatch,
        Self::A4K2IndependentVerify,
    ];

    pub const fn label(self) -> &'static str {
        match self {
            Self::A0AdamW => "A0-adamw",
            Self::A1Sign => "A1-sign",
            Self::A2K2SameBatch => "A2-k2-same-batch",
            Self::A3K5SameBatch => "A3-k5-same-batch",
            Self::A4K2IndependentVerify => "A4-k2-independent-verify",
        }
    }

    const fn beam(self) -> Option<usize> {
        match self {
            Self::A2K2SameBatch | Self::A4K2IndependentVerify => Some(2),
            Self::A3K5SameBatch => Some(5),
            Self::A0AdamW | Self::A1Sign => None,
        }
    }
    ...
    const fn independent_verify(self) -> bool {
        matches!(self, Self::A4K2IndependentVerify)
    }
}
...
pub fn run(samples: &[Sample], arm: Arm, seed: u64) -> RunSummary {
    ...
    for step in 0..STEPS {
        ...
        } else if arm == Arm::A1Sign {
            let (_, gradient) = model.gradient(&proposal_storage);
            update_sign(&mut model, &gradient);
        } else {
            let verify_indices = if arm.independent_verify() {
                batch_indices(seed, step, 0x5645_5249_4659_4241)
            } else {
                proposal_indices
            };
...
mod tests {
    ...
    fn short_runtime_run_commits_a_program() {
        let dataset = spiral_dataset();
        let result = run(&dataset.samples, Arm::A2K2SameBatch, SEEDS[0]);
