...
impl Arm {
    pub const ALL: [Self; 6] = [
        Self::B0MinibatchMinibatch,
        Self::B1MinibatchFull,
        Self::B2FullMinibatch,
        Self::B3FullFull,
        Self::B4AdamW,
        Self::B5Sign,
    ];
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::B0MinibatchMinibatch => "B0-minibatch-minibatch",
            Self::B1MinibatchFull => "B1-minibatch-full",
            Self::B2FullMinibatch => "B2-full-minibatch",
            Self::B3FullFull => "B3-full-full",
            Self::B4AdamW => "B4-adamw",
            Self::B5Sign => "B5-sign",
        }
    }
    ...
    const fn beam(self) -> Option<usize> {
        match self {
            ...
            | Self::B1MinibatchFull
            | Self::B2FullMinibatch
            | Self::B3FullFull => Some(2),
            Self::B4AdamW | Self::B5Sign => None,
        }
    }
    ...
    const fn proposal_full(self) -> bool {
        matches!(self, Self::B2FullMinibatch | Self::B3FullFull)
    }

    const fn verify_full(self) -> bool {
        matches!(self, Self::B1MinibatchFull | Self::B3FullFull)
    }
}
...
pub struct AuditSummary {
    ...
    pub shortlist_regret: f32,
    pub verification_regret: f32,
    pub scheduler_regret: f32,
...
impl AuditSummary {
    fn add(&mut self, other: Self) {
        ...
        self.shortlist_regret += other.shortlist_regret;
        self.verification_regret += other.verification_regret;
        self.scheduler_regret += other.scheduler_regret;
...
fn audit_snapshot(
    ...
    let shortlist_regret = if shortlist_miss > 0 { regret } else { 0.0 };
    let mut verification_noise = 0;
    let mut scheduler_misorders = 0;
    let mut verification_regret = 0.0;
    let mut scheduler_regret = 0.0;
...
pub fn run(samples: &[Sample], arm: Arm, seed: u64) -> RunSummary {
    ...
    for step in 0..STEPS {
        ...
        if arm == Arm::B4AdamW {
        ...
        } else if arm == Arm::B5Sign {
            let (_, gradient) = model.gradient(&proposal_storage);
            update_sign(&mut model, &gradient);
        } else {
            let verify_indices = batch_indices(seed, step, 0x5645_5249_4659_4241);
            let verify_storage = indexed_samples(train, &verify_indices);
            let proposal_batch: &[Sample] = if arm.proposal_full() {
                train
            } else {
                &proposal_storage
            };
            let verify_batch: &[Sample] = if arm.verify_full() {
                train
            } else {
                &verify_storage
            };
            let (groups, group_count) = partition(step % PARAMS);
            let _pair_events = mark_coverage(&mut coverage, &groups, group_count);
            ...
            for &group in &groups[..group_count] {
                let (candidate, candidate_telemetry) =
                    best_runtime_program(&model, proposal_batch, verify_batch, arm, group);
...
mod tests {
    ...
    fn short_runtime_run_commits_a_program() {
        let dataset = spiral_dataset();
        let result = run(&dataset.samples, Arm::B0MinibatchMinibatch, SEEDS[0]);
