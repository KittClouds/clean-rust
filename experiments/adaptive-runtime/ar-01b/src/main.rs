...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        artifact_dir.join("ar-01b-report.json"),
        render_report(&results),
    )?;
    fs::write(artifact_dir.join("ar-01b-runs.csv"), render_runs(&results))?;
    fs::write(
        artifact_dir.join("ar-01b-curves.csv"),
        render_curves(&results),
    )?;
    println!("AR-01B — verification evidence matrix");
    println!(
...
fn render_report(results: &[RunSummary]) -> String {
    ...
    out.push_str("{\n  \"schema\": \"adaptive-runtime-ar-01b/v1\",\n  \"scope\": \"engineering-only\",\n  \"task\": \"three-class-spiral\",\n  \"protocol\": \"B0 minibatch/minibatch; B1 minibatch/full; B2 full/minibatch; B3 full/full; B4 AdamW; B5 sign; 3000 minibatch steps; 3 seeds\",\n  \"results\": [\n");
...
fn render_audit_object(audit: AuditSummary) -> String {
    format!(
        "{{\"snapshots\":{},\"reference_evaluations\":{},\"shortlist_misses\":{},\"verification_noise\":{},\"scheduler_misorders\":{},\"shortlist_regret\":{:.8},\"verification_regret\":{:.8},\"scheduler_regret\":{:.8},\"mean_reference_regret\":{:.8},\"max_reference_regret\":{:.8}}}",
        audit.snapshots,
        audit.reference_evaluations,
        audit.shortlist_misses,
        audit.verification_noise,
        audit.scheduler_misorders,
        audit.shortlist_regret,
        audit.verification_regret,
        audit.scheduler_regret,
...
fn render_runs(results: &[RunSummary]) -> String {
    ...
    for result in results {
        writeln!(
            ...
            result.audit.verification_noise,
            result.audit.scheduler_misorders,
            result.audit.shortlist_regret,
            result.audit.verification_regret,
            result.audit.scheduler_regret,
