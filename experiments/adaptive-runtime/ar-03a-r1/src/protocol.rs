...
pub const SCALES: [f32; 3] = [0.5, 1.0, 2.0];
...
pub const DEVELOPMENT_SEEDS: [u64; 6] = [
    0x1e27_c8d4_6a90_b35f,
    0x64b0_91f2_d37a_58ce,
    0xc319_75a6_0de2_48bf,
    0x7f52_e8a1_349c_06d7,
    0xa83d_26f1_c597_40be,
    0x35e6_b912_8c4f_70ad,
];

pub const EVALUATION_SEEDS: [u64; 6] = [
    0x9d04_7ac2_6e31_b58f,
    0x4f86_d203_b1a9_75ce,
    0xb27c_5e09_83d1_64af,
    0x0a53_f8c6_2d91_47be,
    0x6e19_03d4_a7c2_85bf,
    0xd5a8_41f0_7c36_92eb,
];
...
pub struct Candidate {
    ...
    pub per_example_utility: [Vec<f32>; 3],
}
...
pub fn collect_states(train: &[Sample]) -> Vec<StateSnapshot> {
    ...
    for seed in DEVELOPMENT_SEEDS {
        states.extend(replay_seed(train, seed, StreamRole::Development));
    }
    for seed in EVALUATION_SEEDS {
        states.extend(replay_seed(train, seed, StreamRole::Evaluation));
    }
    states
}

pub fn replay_seed(train: &[Sample], seed: u64, role: StreamRole) -> Vec<StateSnapshot> {
    ...
    for (block, group) in groups.iter().copied().enumerate() {
        ...
        let all_legal = left_by_rank.iter().all(|&left_index| {
            SCALES.iter().all(|scale| {
                (LOWER_BOUND..=UPPER_BOUND)
                    .contains(&(model.parameters[group.left] + *scale * ACTION_VALUES[left_index]))
            })
        }) && right_by_rank.iter().all(|&right_index| {
            SCALES.iter().all(|scale| {
                (LOWER_BOUND..=UPPER_BOUND).contains(
                    &(model.parameters[group.right] + *scale * ACTION_VALUES[right_index]),
                )
            })
        });
        if !all_legal {
        ...
        for (left_rank, &left_index) in left_by_rank.iter().enumerate() {
            for (right_rank, &right_index) in right_by_rank.iter().enumerate() {
                ...
                let mut utilities: [Vec<f32>; 3] =
                    std::array::from_fn(|_| Vec::with_capacity(TRAIN_SAMPLES));
                let mut exact_utility = [0.0; 3];
                for (scale_index, &scale) in SCALES.iter().enumerate() {
                    let mut candidate_model = *model;
                    candidate_model.parameters[group.left] += scale * left_delta;
                    candidate_model.parameters[group.right] += scale * right_delta;
                    let values = &mut utilities[scale_index];
                    for (sample_index, sample) in train.iter().enumerate() {
                        values.push(baseline[sample_index] - candidate_model.loss_one(sample));
                    }
                    exact_utility[scale_index] =
                        f64::from(values.iter().sum::<f32>() / TRAIN_SAMPLES as f32);
                }
...
mod tests {
    ...
    fn candidate_action_split_is_balanced_and_scale_legal() {
        let train = crate::model::generate_dataset();
        let proposal: [Sample; BATCH_SIZE] =
