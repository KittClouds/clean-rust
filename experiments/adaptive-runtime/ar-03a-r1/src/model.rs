...
pub const DATASET_SEED: u64 = 0x72d8_4f31_a06c_b59e;
const MAGIC: &[u8; 8] = b"AR03R1D1";
...
impl MappedDataset {
    pub fn generate_write_open(path: impl AsRef<Path>) -> io::Result<Self> {
    ...
    pub fn open(path: impl AsRef<Path>) -> io::Result<Self> {
        ...
        if mmap.len() < HEADER_BYTES || memmem::find(&mmap[..8], MAGIC) != Some(0) {
            return Err(io::Error::new(
                ...
                "AR-03A-R1 dataset magic mismatch",
            ));
        }
            ...
            || mmap.len() != HEADER_BYTES + TRAIN_SAMPLES * size_of::<Sample>()
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "AR-03A-R1 dataset layout mismatch",
            ));
        }
        Ok(Self { mmap })
    }
...
impl Model {
    pub fn initial() -> Self {
        ...
        for (index, value) in parameters.iter_mut().enumerate() {
            let phase = (index as f32 * 1.3714).sin();
...
pub(crate) fn generate_dataset() -> [Sample; TRAIN_SAMPLES] {
    let mut rng = NormalRng::new(DATASET_SEED);
    std::array::from_fn(|_| {
