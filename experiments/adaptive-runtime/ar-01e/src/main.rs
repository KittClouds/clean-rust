...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        artifact_dir.join("ar-01e-report.json"),
        render_report(&results),
    )?;
    fs::write(artifact_dir.join("ar-01e-runs.csv"), render_runs(&results))?;
    fs::write(
        artifact_dir.join("ar-01e-curves.csv"),
        render_curves(&results),
    )?;
    println!("AR-01E — equal-total-commit evidence cadence");
    println!(
...
fn render_report(results: &[RunSummary]) -> String {
    ...
    out.push_str("{\n  \"schema\": \"adaptive-runtime-ar-01e/v1\",\n  \"scope\": \"engineering-only\",\n  \"task\": \"three-class-spiral\",\n  \"protocol\": \"E0-E3 same-batch, E4-E7 independent-batch, E8-E11 full-train verification; each crossed with 1/2/4/8 commits per evidence refresh; E12 AdamW; E13 sign; proposal evidence is always the current minibatch; total commit budget 4800; 3 seeds\",\n  \"results\": [\n");
