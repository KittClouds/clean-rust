...
const PARENT_COMMIT: &str = "f850f537e15e28024cca163b43ad6e4052b2b535";
const STREAMS_PER_CELL: usize = 2;
...
pub fn run(output_dir: &Path) -> io::Result<RunReport> {
    ...
    let mut trajectories = Vec::with_capacity(9 * STREAMS_PER_CELL * RuntimeArm::ALL.len());
...
fn write_cell_summary(output_dir: &Path, trajectories: &[MeasuredTrajectory]) -> io::Result<()> {
    ...
    let mut cells = CellAggregates {
        projected_losses: Vec::with_capacity(9),
        hash_losses: Vec::with_capacity(9),
        paired_differences: Vec::with_capacity(9),
        projected_accuracies: Vec::with_capacity(9),
        hash_accuracies: Vec::with_capacity(9),
        projected_train_losses: Vec::with_capacity(9),
        hash_train_losses: Vec::with_capacity(9),
    };
    for cell in 0..9 {
        let cell_rows: Vec<_> = trajectories
            .iter()
            .filter(|row| {
                usize::from(row.trace.dataset_index) * 3
...
fn write_integrity(
    ...
    writeln!(out, "{{")?;
    writeln!(out, "  \"protocol\": \"AR-03D-2026-09-21\",")?;
    ...
    writeln!(out, "  \"dataset_initialization_cells\": 9,")?;
