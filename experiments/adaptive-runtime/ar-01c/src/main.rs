...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        artifact_dir.join("ar-01c-report.json"),
        render_report(&results),
    )?;
    fs::write(artifact_dir.join("ar-01c-runs.csv"), render_runs(&results))?;
    fs::write(
        artifact_dir.join("ar-01c-curves.csv"),
        render_curves(&results),
    )?;
    println!("AR-01C — adaptation bandwidth");
    println!(
...
fn render_report(results: &[RunSummary]) -> String {
    ...
    out.push_str("{\n  \"schema\": \"adaptive-runtime-ar-01c/v1\",\n  \"scope\": \"engineering-only\",\n  \"task\": \"three-class-spiral\",\n  \"protocol\": \"C0/C1/C2/C3 use K2 same-batch with 1/2/4/8 sequential commits per minibatch; C4 AdamW; C5 sign; 600 evidence rounds; 3 seeds\",\n  \"results\": [\n");
