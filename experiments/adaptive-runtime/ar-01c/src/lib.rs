...
impl Arm {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::C0OneCommit => "C0-one-commit",
            Self::C1TwoCommits => "C1-two-commits",
            Self::C2FourCommits => "C2-four-commits",
            Self::C3EightCommits => "C3-eight-commits",
            Self::C4AdamW => "C4-adamw",
            Self::C5Sign => "C5-sign",
        }
    }
    ...
    const fn beam(self) -> Option<usize> {
        match self {
            Self::C0OneCommit | Self::C1TwoCommits | Self::C2FourCommits | Self::C3EightCommits => {
                Some(2)
            }
            Self::C4AdamW | Self::C5Sign => None,
        }
    }
    ...
    const fn commits_per_evidence(self) -> usize {
        match self {
            Self::C0OneCommit => 1,
            Self::C1TwoCommits => 2,
            Self::C2FourCommits => 4,
            Self::C3EightCommits => 8,
            Self::C4AdamW | Self::C5Sign => 1,
        }
    }
}
...
pub fn run(samples: &[Sample], arm: Arm, seed: u64) -> RunSummary {
    ...
    for step in 0..STEPS {
        ...
        } else if arm == Arm::C5Sign {
            let (_, gradient) = model.gradient(&proposal_storage);
            update_sign(&mut model, &gradient);
        } else {
            let verify_storage = proposal_storage;
...
mod tests {
    ...
    fn short_runtime_run_commits_a_program() {
        let dataset = spiral_dataset();
        let result = run(&dataset.samples, Arm::C2FourCommits, SEEDS[0]);
