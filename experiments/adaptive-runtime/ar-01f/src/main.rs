...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        artifact_dir.join("ar-01f-report.json"),
        render_report(&results),
    )?;
    fs::write(artifact_dir.join("ar-01f-runs.csv"), render_runs(&results))?;
    fs::write(
        artifact_dir.join("ar-01f-curves.csv"),
        render_curves(&results),
    )?;
    println!("AR-01F — independent verification evidence frontier");
    println!(
...
fn render_report(results: &[RunSummary]) -> String {
    ...
    out.push_str("{\n  \"schema\": \"adaptive-runtime-ar-01f/v1\",\n  \"scope\": \"engineering-only\",\n  \"task\": \"three-class-spiral\",\n  \"protocol\": \"F0-F5 use independent verification samples of 16/24/32/48/64/96 with K2 and 8 sequential commits per evidence sample; F6 AdamW; F7 sign; proposal evidence is always a 16-example minibatch; 4800 total commits; 3 seeds\",\n  \"results\": [\n");
