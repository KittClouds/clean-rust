...
use g_reasoner_34m_parity::kernel::{fast_distmult_sum, scalar_distmult_sum};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let nodes = env_usize("G_REASONER_PERF_NODES", 5_000);
    let edges_per_node = env_usize("G_REASONER_PERF_EDGES_PER_NODE", 4);
    let iterations = env_usize("G_REASONER_PERF_ITERATIONS", 5);
    let relations = 64;
    let dim = 1024;
    ...
    println!(
        "{{\"nodes\":{nodes},\"edges\":{},\"dim\":{dim},\"kernel\":\"{:?}\",\"scalar_ms\":{:.3},\"fast_median_ms\":{:.3},\"speedup\":{:.3},\"max_abs\":{max_abs:.9}}}",
        graph.edge_count(),
        kernel.unwrap(),
        millis(scalar),
        millis(median),
