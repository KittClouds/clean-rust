...
#[test]
fn espionage_reframe_proves_theft_performance_distrust_exposure_and_sabotage() {
    let (gold, projections) = gold_fixtures();
    let gold_case = gold
        .cases
        .iter()
        .find(|case| case.case_id.0 == "gold:removed_relationship")
        .unwrap();
    ...
    let result = execute(simulation_input(&fixture, &report, &mutations), &edit)
        .expect("Hazel espionage shadow repair");
    ...
    assert_eq!(result.semantic_deltas.len(), 39);
    assert_eq!(result.receipt.outcomes.len(), 4);
    ...
    let ledgers = result
        ...
        .filter_map(|delta| match delta {
            SemanticDelta::AddRelationshipLedger { record } => Some(record),
            _ => None,
        })
        .collect::<Vec<_>>();
    ...
    assert!(ledgers
        .iter()
        .any(|ledger| ledger.relation_family == "performed_alliance"));
    assert!(ledgers
        .iter()
        .any(|ledger| ledger.relation_family == "mutual_distrust"));
    assert_eq!(
        result
            .semantic_deltas
            .iter()
            .filter(|delta| matches!(delta, SemanticDelta::AddBeliefAtom { .. }))
            .count(),
        2
    );
}
...
#[test]
fn espionage_shadow_execution_is_deterministic() {
    let (gold, projections) = gold_fixtures();
    let gold_case = &gold.cases[5];
    assert_eq!(gold_case.case_id.0, "gold:removed_relationship");
    let projection = projection_case(&projections.cases, gold_case);
    let fixture = case_fixture(gold_case, projection);
    let report = report_for(gold_case, projection);
    let edit = preferred_edit(&report);
...
#[test]
fn restoring_confidant_status_downgrades_the_candidate_to_unknown() {
    let (gold, projections) = gold_fixtures();
    let gold_case = &gold.cases[5];
    let projection = projection_case(&projections.cases, gold_case);
    let fixture = case_fixture(gold_case, projection);
    ...
    let mutations = [StoryMutation::ChangeState {
        ...
        state_kind: StateKind("relationship:silas".into()),
        replacement: FactValue::Text("confidant".into()),
    ...
    assert!(result.receipt.outcomes.iter().any(|outcome| {
        outcome.outcome_id == "confidant_access_is_replaced_by_theft" && !outcome.satisfied
    }));
    assert!(result.receipt.no_truth_writes);
}
...
fn preferred_edit(report: &RevisionImpactReport) -> ProposedEdit {
    ...
    let directive = directives
        .for_case("gold:removed_relationship")
        .expect("removed relationship directive");
