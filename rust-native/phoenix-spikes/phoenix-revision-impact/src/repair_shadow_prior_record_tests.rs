...
use phoenix_types::{FactValue, RevisionImpactGoldCase, StateKind, StoryMutation, StoryTime};
...
#[test]
fn earlier_death_shadow_rebuild_proves_recording_legacy_and_false_rumor() {
    let (gold, projections) = gold_fixtures();
    let gold_case = gold
        .cases
        .iter()
        .find(|case| case.case_id.0 == "gold:earlier_death")
        .unwrap();
    ...
    let result = execute(simulation_input(&fixture, &report, &mutations), &edit)
        .expect("prior-record shadow repair");
    ...
    assert_eq!(result.semantic_deltas.len(), 20);
    ...
    assert!(result.semantic_deltas.iter().any(|delta| matches!(
        ...
        SemanticDelta::ResolveCoverageRequirement {
            ...
            ..
        } if scene_id.0 == RUMOR_SCENE
            && removed_required_planes.contains(&CoveragePlane::Capability)
    )));
}

#[test]
fn earlier_death_shadow_execution_is_deterministic() {
    let (gold, projections) = gold_fixtures();
    let gold_case = &gold.cases[1];
    assert_eq!(gold_case.case_id.0, "gold:earlier_death");
    let projection = projection_case(&projections.cases, gold_case);
    let fixture = case_fixture(gold_case, projection);
    let report = report_for(gold_case, projection);
    let edit = preferred_edit(&report);
...
#[test]
fn moving_maras_death_boundary_downgrades_the_candidate_to_unknown() {
    let (gold, projections) = gold_fixtures();
    let gold_case = &gold.cases[1];
    let projection = projection_case(&projections.cases, gold_case);
    let fixture = case_fixture(gold_case, projection);
    ...
    let mutations = [StoryMutation::ChangeState {
        ...
        state_kind: StateKind("alive".into()),
        replacement: FactValue::Boolean(false),
        valid_from: StoryTime(750),
    }];
    ...
    assert!(result
        ...
        .iter()
        .any(
            |outcome| outcome.outcome_id == "mara_definitively_dead_at_700" && !outcome.satisfied
        ));
    assert!(result.receipt.no_truth_writes);
}
...
fn preferred_edit(report: &RevisionImpactReport) -> ProposedEdit {
    ...
    let directive = directives
        .for_case("gold:earlier_death")
        .expect("earlier death directive");
