...
use phoenix_lexical_qps::{
    GroupStrengthBatch, RankEvidenceV3, RelevanceTier, SearchHit, SearchScratch,
};
use serde::{Deserialize, Serialize};

use super::*;

const CONTRACT: &str = "phoenix.memory.qps-v3-phase8.9-release-group-distribution-sidecar/v1";
...
pub(crate) fn capture(
    ...
    gold_path: &Path,
    output_path: &Path,
) -> Result<ReleaseGroupDistributionPublication> {
    ...
    {
        bail!("release group-distribution capture requires verified Phase 3");
    }
    ...
    let gates = ReleaseGroupDistributionGates {
        ...
        strengths_are_finite_and_bounded: mixed
            .queries
            .iter()
            .chain(&longmemeval.queries)
            .flat_map(|query| &query.candidates)
            .flat_map(|candidate| &candidate.strengths)
            .all(|value| value.is_finite() && (0.0..=1.0).contains(value)),
        profile_group_counts_match_evidence: mixed
            .queries
            .iter()
            .chain(&longmemeval.queries)
            .flat_map(|query| &query.candidates)
            .all(|candidate| {
                candidate.strengths.len() == candidate.rank_evidence_v3.query_groups as usize
            }),
    };
    if !gates.all_pass() {
        bail!("release group-distribution parity failed: {gates:?}");
    }
    let artifact = ReleaseGroupDistributionSidecar {
        contract: CONTRACT,
        schema_version: 1,
    ...
    Ok(ReleaseGroupDistributionPublication {
        contract: CONTRACT,
...
fn capture_mixed(suite: &MixedSuite) -> Result<GroupDistributionCohort> {
    ...
    let mut strengths = GroupStrengthBatch::with_capacity(CANDIDATE_CAP, MAXIMUM_QUERY_GROUPS);
    for query in &suite.queries {
        ...
        index.capture_group_strengths_into(&scratch, &hits, &mut strengths)?;
        let candidates = hits
            .iter()
            .enumerate()
            .map(|(rank, hit)| {
                Ok(GroupDistributionCandidate {
                    ...
                    strengths: strengths
                        .strengths(rank)
                        .context("mixed group-strength row missing")?
                        .to_vec(),
                })
            })
            .collect::<Result<Vec<_>>>()?;
        queries.push(GroupDistributionQuery {
            query_identity: query.stable_id.clone(),
            candidates,
        });
    }
    Ok(GroupDistributionCohort { queries })
}

fn capture_longmemeval(workload: &WorkloadArtifact) -> Result<GroupDistributionCohort> {
    ...
    for case in &workload.cases {
        ...
        let candidates = hits
            ...
            .enumerate()
            .map(|(rank, hit)| {
                let document = hit.external_id as usize;
                Ok(GroupDistributionCandidate {
                    document_identity: case.sessions[document].stable_id.clone(),
                    document_version: versions[document].clone(),
                    v2_order: rank + 1,
                    v2_score_bits: hit.v2_score.to_bits(),
                    rank_evidence_v3: hit.rank_evidence_v3,
                    relevance_tier: hit.relevance_tier,
                    strengths: strengths
                        .strengths(rank)
                        .context("LongMemEval group-strength row missing")?
                        .to_vec(),
                })
            })
            .collect::<Result<Vec<_>>>()?;
...
fn compare_cohort(expected: &FrozenCohort, actual: &GroupDistributionCohort) -> CohortParity {
...
struct ReleaseGroupDistributionSidecar {
    contract: &'static str,
    schema_version: u16,
    ...
    gold: FileIdentity,
    mixed: GroupDistributionCohort,
    longmemeval: GroupDistributionCohort,
    gates: ReleaseGroupDistributionGates,
...
#[derive(Debug, Serialize)]
struct GroupDistributionCohort {
    queries: Vec<GroupDistributionQuery>,
}

#[derive(Debug, Serialize)]
struct GroupDistributionQuery {
    query_identity: String,
    candidates: Vec<GroupDistributionCandidate>,
}

#[derive(Debug, Serialize)]
struct GroupDistributionCandidate {
...
pub(super) struct ReleaseGroupDistributionGates {
...
impl ReleaseGroupDistributionGates {
...
#[derive(Debug, Serialize)]
pub(crate) struct ReleaseGroupDistributionPublication {
    contract: &'static str,
    output: FileIdentity,
    gates: ReleaseGroupDistributionGates,
    production_promotion_authorized: bool,
