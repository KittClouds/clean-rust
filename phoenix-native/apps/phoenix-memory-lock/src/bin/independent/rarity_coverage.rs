...
const CONTRACT: &str = "phoenix.qps.phase8.7-rarity-group-coverage-sidecar/v1";

#[derive(Debug, Serialize)]
pub(super) struct RarityCoverageCapture {
    #[serde(skip)]
    enabled: bool,
    pairs: Vec<PairRarityCoverage>,
    graded_queries: Vec<GradedQueryRarityCoverage>,
}

impl RarityCoverageCapture {
    ...
    pub(super) fn capture_pair(
        ...
        self.pairs.push(PairRarityCoverage {
            ...
            positive_rarity_weighted_group_coverage: positive.rarity_weighted_group_coverage,
            negative_rarity_weighted_group_coverage: negative.rarity_weighted_group_coverage,
    ...
    pub(super) fn capture_graded(
        ...
        candidates: &[GradedCandidateRarityCoverage],
    ) {
        if self.enabled {
            self.graded_queries.push(GradedQueryRarityCoverage {
                query_identity,
    ...
    pub(super) fn write(self, path: &Path, ledger_path: &Path, graded_path: &Path) -> Result<()> {
        ...
        let artifact = RarityCoverageSidecar {
            contract: CONTRACT,
            schema_version: 1,
            architecture:
                "diagnostic_only_replace_redundant_slot_8_with_rarity_weighted_group_coverage",
            group_weight:
...
#[derive(Debug, Serialize)]
struct GradedQueryRarityCoverage {
    query_identity: String,
    candidates: Vec<GradedCandidateRarityCoverage>,
}
...
fn write_json_atomic<T: Serialize>(path: &Path, value: &T) -> Result<()> {
    ...
    let parent = path
        .parent()
        .context("rarity coverage sidecar has no parent")?;
